...
class NeuralOC:
  ...
  def _get_step_fn(self) -> Callable:
      ...
      def am_loss_sample(state, params, key_t, t_sample, x_sample, target_state):
        ...
        dsdt, dsdx = dsdtdx_fn(params, t, x_t)
        dsdt_tgt, dsdx_tgt = dsdtdx_fn(target_state.params, t, x_t)
        u = dsdx_tgt
        vt = dsdt_tgt
      ...
      def potential_loss(state, params, key, source, target):
        ...
        result = TimedX(
           result.t.reshape(-1),
           result.x.reshape(-1, D)
        )

        if self.backward_batch_size > 0:

          B = self.backward_batch_size
          key, key_back = jax.random.split(key)
          _, result_back = jax.lax.scan(move_back, (t_1[:B], x_1[:B], key_back), None, length=self.backward_steps)
      ...
      def train_step_cost(state, key, source, target, t_sample, x_sample, target_state, scale_ema):
        ...
        (loss_potential, x_seq), potential_grads = grad_fn(state, state.params, key, source, target)
