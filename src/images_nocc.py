...
class ResNetDwTime(nn.Module):
    ...
    def __call__(self, t, x, train=True):
        ...
        t_pos = t * jnp.arange(1, 20)
        t_pos = jnp.concatenate([jnp.sin(t_pos), jnp.cos(t_pos)], -1)
        t_pos = nn.Dense(self.size**2)(t_pos) # [b, size**2]
        t_pos = t_pos.reshape(b_size, 1, self.size, self.size)

        x_with_t = jnp.concatenate([x, t_pos], axis=1)
        return ResNet_D(
            size=self.size,
